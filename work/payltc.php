...
$_SESSION['email'];
// echo $email;
$url = 'https://sandbox.cryptapi.io/btc/create/?address='.$ad.'&callback='.$ur.'/callback?invoice='.$h.'';
//  Initiate curl
$ch = curl_init();
...
$result=mysqli_query($link,$search)  or die(mysqli_error($link));
 ...
 if($match < 1){
    ...
    $sql = "INSERT INTO users ( AccountNumber, BankCode, currency, Amount, AccountName, in_address, notify_confirmations, value_forwarded_coin, txid_out, txid_in, email ) VALUES ('$AccountNumber', '$BankCode', '$currency', '$Amount', '$AccountName', '$in_address', '$notify_confirmations', '$value_forwarded_coin', '$txid_out', '$txid_in', '$email')";
...
<html lang="en">
  ...
  <li class="list-group-item">Amount to Send<span  style="float:right;" style="font-style: Garamond;" id="pick" ><?Php ?> <span>BTC    <i class="fas fa-copy" id="char" onclick="CopyTo('char');return false;" style="color: blue;"></i></span></span></li>
   <span id="sharp" style="display: none;" class="text-success">Copied!</span>
  <li class="list-group-item">Recipient will recieve<span style="float: right;">NGN <span><?Php echo $_SESSION['amount']; ?></span></span></li>
