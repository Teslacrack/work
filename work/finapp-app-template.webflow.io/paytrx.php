...
// include 'percent.php';
include 'naira.php';
include 'trxprice.php';
include 'trxaddress.php';
$invoice=$_SESSION['invoice'];


//address
$url=('https://api.cryptapi.io/trx/create/?address=TG2XUqp9R1PJeCN1YtjWPRrrTkYD2FMSSv&callback=http://ego.ezyro.com/work/finapp-app-template.webflow.io/paybtc.php/?invoice='.$invoice.'&nonce=randomstring');
//  Initiate curl
$ch = curl_init();
...
$cryptotodollar=($in['USD']);
$dollarnairaratio= 392; 
$numberofcrypto= floatval($Amount);
 $valueindollar=$cryptotodollar* $numberofcrypto;
 $subtracted = $valueindollar *$dollarnairaratio;
//xchange
...
$result=mysqli_query($link,$search)  or die(mysqli_error($link));
 ...
 if($match < 1){
    ...
    if(isset($AccountNumber) && isset($BankCode) && isset($currency) && isset($Amount) && isset($AccountName) && isset($in_address)  && isset($email) && isset($subtracted) && isset($invoice) ) {
        $sql = "INSERT INTO users ( AccountNumber, BankCode, currency, Amount, AccountName, in_address, email, subtracted, invoice ) VALUES ('$AccountNumber', '$BankCode', '$currency', '$Amount', '$AccountName', '$in_address', '$email', '$subtracted', '$invoice')";
...
<html lang="en">
  ...
  <li class="list-group-item">You will recieve<span style="float: right;">NGN <span><?Php echo $subtracted; ?></span></span></li>
...
<P style="text-align: center;" id="address"> <?Php echo $in_address  ?> </P>
